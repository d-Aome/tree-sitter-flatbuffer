package io.github.treesitter.jtreesitter.flatbuffer;

import java.lang.foreign.*;

public final class TreeSitterFlatbuffer {
    ...
    private static final TreeSitterFlatbuffer INSTANCE = new TreeSitterFlatbuffer();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterFlatbuffer() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_flatbuffer");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-flatbuffer");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-flatbuffer");
